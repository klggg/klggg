...
//多语言变量定义,以下代码不能改
var div_page_str1="提示";

//close_image:传一个参数，非数字变可以了
//functions：方法或链接，close_image与functions给合在一起用的，用于X的按钮
function div_page(url, title, win_w, win_h, close_image, functions) {
	var i;
	...
	var close_image_onclick = "javascript:div_page_close();";
	
	if(close_image && functions && isNaN(close_image) && isNaN(functions)){
		
			if (functions.indexOf("js:") == 0) {
				close_image_onclick ="javascript:"+functions.substr(3);
			}
			else {
				close_image_onclick = "javascript:window.location.href='"+functions+"'";
			}
	...
	if (document.compatMode == "BackCompat") {
		cWidth = documsent.body.clientWidth;
...
function div_page_close() {
	...
	try{
		if(document.getElementById('div_page')){
			div_close_func();
			div_close_func = null;
...
function div_page1(url, title,close_image, functions) {
	
	div_page(url, title, 680, 420,close_image, functions);
	
}
function div_page2(url, title, close_image, functions) {
	
	div_page(url, title, 580, 370, close_image, functions);
	
}
function div_page3(url, title,close_image, functions) {
	
	div_page(url, title, 400, 200,close_image, functions);
	
}

function Div_Mission(id){
	div_page("../mission/mission_tips.php?id="+id,'',535,190);
}

function Div_Mission2(id){
	div_page("./mission/mission_tips.php?id="+id,'',535,190);
}

function div_page_mission(url, title, win_w, win_h) {
	var i;
	...
	div_page.style.width = win_w + "px";
	div_page.style.height = win_h + "px";
	// div_page.style.top = (sTop+20 + (cHeight - win_h) / 2) + "px";
	// div_page.style.left = (sLeft + (cWidth - win_w) / 2) + "px";
	div_page.style.top = 36 + "px";
	div_page.style.left = 2 + "px";
