...
function syn_deal_status($id)
{
	...
	if($deal_info['deal_status']!=3){
		...
		if($deal_info['progress_point'] >=100 || $data['progress_point'] >=100){
			...
			if($GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal_inrepay_repay WHERE deal_id=$id") >0){
				$data['deal_status'] = 5;
				$repay_info =  $GLOBALS['db']->getRow("SELECT sum(repay_money) As all_repay_money  FROM ".DB_PREFIX."deal_repay WHERE deal_id=$id");
				if($repay_info){
	...
	if(($deal_info['deal_status'] == 1 || $data['deal_status']==1) && $deal_info['remain_time'] >0 && $deals_time - $deal_info['start_time'] >=1800){
		//point
		...
		$sql = "SELECT usa.user_id,usa.fixed_amount,u.user_name FROM ".DB_PREFIX."user_autobid usa LEFT JOIN ".DB_PREFIX."user u ON u.id=usa.user_id " .
				...
				"AND usa.user_id not in (SELECT user_id FROM ".DB_PREFIX."deal_load WHERE deal_id=$id) and usa.user_id <>'".$deal_info['user_id']."' " .
				...
				"AND (usa.fixed_amount <= ".$deal_info['max_loan_money']." or ".$deal_info['max_loan_money']." = 0)".
