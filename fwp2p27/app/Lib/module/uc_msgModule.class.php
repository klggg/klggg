...
class uc_msgModule extends SiteBaseModule
{
	...
	{	
		...
		if($mtype=="private"){
			$GLOBALS['tmpl']->assign("page_title",$GLOBALS['lang']['UC_PRIVATE_MSG']);
	...
	public function deal()
	{
		...
		elseif($row['system_msg_id']>0||$row['is_notice']>=1)
		{
			...
			$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."msg_box where system_msg_id = ".$row['system_msg_id']." AND is_notice=".$row['is_notice']." AND to_user_id=".$GLOBALS['user_info']['id']." and is_delete = 0 ORDER BY id DESC LIMIT ".$limit);
			$GLOBALS['tmpl']->assign("list",$list);	
			
			$total = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."msg_box where system_msg_id = ".$row['system_msg_id']." AND is_notice=".$row['is_notice']." AND to_user_id=".$GLOBALS['user_info']['id']." and is_delete = 0");
			$page = new Page($total,app_conf("PAGE_SIZE"));   //初始化分页对象 		
			$p  =  $page->show();
			$GLOBALS['tmpl']->assign('pages',$p);
			
			$GLOBALS['db']->query("update ".DB_PREFIX."msg_box set is_read = 1 where system_msg_id = ".$row['system_msg_id']." AND is_notice=".$row['is_notice']);
